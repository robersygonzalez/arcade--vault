...
- **Acoplamiento al DOM**: `document.getElementById('score')`, `#overlay`, `#restart-btn`, toggle de tema ad-hoc del juego original con `localStorage` (light/dark del propio `game.js`) — todo esto se recorta. El HUD, el modal de fin de partida y el reinicio ya los provee `game-player.tsx`. Esto **no** aplica al sistema de skins de la plataforma (`clasico`/`neon`/`retro`, ver el agente `skin-designer` y `components/games/skins.ts` si ya existe) — ese es un mecanismo distinto, añadido después del porteo, y no debe confundirse con el toggle que se recorta aquí.
